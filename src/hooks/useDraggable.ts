...
export function useDraggable(options: DraggableOptions) {
    ...
    const updatePosition = useCallback((clientX: number, clientY: number) => {
        ...
        // Get the window element's actual height
        const elHeight = elementRef.current?.offsetHeight || 200;

        // Prevent dragging off screen
        // Bottom: window's bottom edge can't go past the taskbar
        const maxY = window.innerHeight - taskbarHeight - elHeight;
        // Right: keep at least 100px visible horizontally
