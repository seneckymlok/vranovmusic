    ...
    const handleMouseMove = useCallback((e: MouseEvent | TouchEvent) => {
        ...
        if (resizeDirection.includes('s')) {
            ...
            const taskbarHeight = parseInt(
                getComputedStyle(document.documentElement).getPropertyValue('--taskbar-height') || '40',
                10
            );
            const maxAvailableHeight = window.innerHeight - taskbarHeight - startPosition.current.y;
            const effectiveMaxHeight = Math.min(maxSize.height, maxAvailableHeight);
            newHeight = Math.min(effectiveMaxHeight, Math.max(minSize.height, startSize.current.height + deltaY));
        }
