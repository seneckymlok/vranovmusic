...
export const Window: React.FC<WindowProps> = ({
    ...
    const getWindowStyle = (): React.CSSProperties => {
        ...
        // Clamp height so window never extends below the taskbar
        const taskbarHeight = parseInt(
            getComputedStyle(document.documentElement).getPropertyValue('--taskbar-height') || '40',
            10
        );
        const maxHeight = window.innerHeight - taskbarHeight - currentPosition.y;
        const clampedHeight = Math.min(currentSize.height, Math.max(100, maxHeight));
